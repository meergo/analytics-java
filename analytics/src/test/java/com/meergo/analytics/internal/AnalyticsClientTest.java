...
import static com.meergo.analytics.internal.FlushMessage.POISON;
import static com.meergo.analytics.internal.StopMessage.STOP;
...
import com.meergo.analytics.TestUtils.MessageBuilderTest;
import com.meergo.analytics.http.MeergoService;
import com.meergo.analytics.http.UploadResponse;
import com.meergo.analytics.internal.AnalyticsClient.BatchUploadTask;
import com.meergo.analytics.messages.Batch;
...
public class AnalyticsClientTest {
  ...
  @Mock MeergoService meergoService;
  ...
  public void batchRetriesForNetworkErrors() {
    ...
    when(meergoService.upload(null, batch))
    ...
    verify(meergoService, times(4)).upload(null, batch);
  ...
  public void batchDoesNotRetryForNon5xxAndNon429HTTPErrors() {
    ...
    when(meergoService.upload(null, batch)).thenReturn(Calls.response(failResponse));

    BatchUploadTask batchUploadTask = new BatchUploadTask(client, BACKO, batch, DEFAULT_RETRIES);
    batchUploadTask.run();

    // Verify we only tried to upload once.
    verify(meergoService).upload(null, batch);
  ...
  public void batchDoesNotRetryForNonNetworkErrors() {
    ...
    when(meergoService.upload(null, batch)).thenReturn(networkFailure);
  ...
  public void givesUpAfterMaxRetries() {
    ...
    verify(meergoService, times(11)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
  ...
  public void shutdownWhenAlreadyShutDown() throws InterruptedException {
    ...
    verify(messageQueue, times(0)).put(any(Message.class));
    verifyNoInteractions(networkExecutor, callback, meergoService);
  }
  ...
  public void neverRetries() {
    ...
    verify(meergoService, times(1)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
